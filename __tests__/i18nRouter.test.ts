...
import { RoutingStrategies } from '../src/types';

const basePaths = ['', '/with-basepath'];
const routingStrategies: RoutingStrategies[] = ['rewrite', 'dynamicSegment'];

basePaths.forEach(basePath => {
  routingStrategies.forEach(routingStrategy => {
    describe(`i18nRouter${basePath ? ' with basePath' : ''}`, () => {
      ...
      it('should throw an error if defaultLocale is not defined', () => {
        const config = { locales: ['en-US'], basePath, routingStrategy };
